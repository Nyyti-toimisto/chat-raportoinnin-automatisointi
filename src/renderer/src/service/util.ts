...
export const separateBooleanAnswers = (postFeedbacks: PostFeedBackQuestionSummary[]) => {
    ...
    const booleansFiltered = postFeedbacks.filter((feedback) => {
        if (
            Object.keys(feedback.answers).length === 1 &&
    ...
    const booleanStructrued = [
        {
            ...
            answers: booleansFiltered.reduce((acc, val) => {
                ...
                if (!acc[hash]) {
                    acc[hash] = {
                        answer: val.question,
                        count: Object.values(val.answers)[0].count
                    };
                }
                return acc;
            }, {})
